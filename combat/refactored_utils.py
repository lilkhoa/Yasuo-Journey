...
def load_grid_sprite_sheet(factory, filepath, cols, rows, target_size=None):
    if not os.path.exists(filepath):
        print(f"[UTILS-ERROR] Không tìm thấy file: {filepath}")
        return []
...
def load_image_sequence(factory, folder_path, prefix, count, target_size=None, zero_pad=True):
    """
    Load chuỗi ảnh rời.
    - zero_pad=True: prefix001.png, prefix002.png...
    - zero_pad=False: prefix1.png, prefix2.png...
    """
    if not os.path.exists(folder_path):
        print(f"[UTILS-ERROR] Không tìm thấy thư mục: {folder_path}")
        return []

    sprites = []

    for i in range(1, count + 1):
        filename = f"{prefix}{i:03d}.png" if zero_pad else f"{prefix}{i}.png"
...
def flip_sprites_horizontal(factory, original_sprites):
    ...
    for sprite in original_sprites:
        ...
        if not dup_surface_ptr: continue
