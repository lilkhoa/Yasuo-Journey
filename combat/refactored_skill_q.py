...
from combat.refactored_utils import load_grid_sprite_sheet
...
class TornadoObject:
    ...
    def delete(self): self.entity.delete()

# --- REFACTORED CLASS ---
class SkillQ(BaseSkill):
    """Bản chất là TornadoSkill"""
    def __init__(self, owner):
        super().__init__(owner, name="Tornado", base_cooldown=0.1)
        
    def execute(self, world, factory, renderer, skill_sprites=None, skill_surface=None, **kwargs):
        ...
        tornado = TornadoObject(world, sprites_to_use, start_x, start_y, 
                                direction, max_dist=700, max_hits=99, damage_multiplier=self.damage_multiplier)
        return tornado

# Logic Update rời vẫn được giữ để game loop cũ không bị gãy (Sẽ gọi trong update_skills của Yasuo)
def update_q_logic(tornado_obj, enemies, dt, network_ctx=None):
    ...
    for eid in tornado_obj.hit_records:
        ...
        if tornado_obj.hit_records[eid] <= 0: to_remove.append(eid)
    for eid in to_remove: del tornado_obj.hit_records[eid]
    ...
    for target in enemies:
        ...
        if sdl2.SDL_HasIntersection(tornado_rect, target_rect):
            ...
            if target_net_id not in tornado_obj.hit_records:
                current_dmg = tornado_obj.damage_base 
                ...
                if network_ctx:
                    ...
                    if is_multi and game_client and game_client.is_connected():
                        ...
                        if hasattr(target, 'take_damage'): target.take_damage(current_dmg)
                else:
                    if hasattr(target, 'take_damage'): target.take_damage(current_dmg)
                
                # Knockup Fallback
                if hasattr(target, 'apply_knockup'): target.apply_knockup(-12)
                elif hasattr(target, 'velocity_y'): target.velocity_y = -10
